...
<html>
<head>
<title>Ejercicio 2 - Tarea 5</title>
<h1>Todas las recetas veganas</h1>
<link rel="stylesheet" type="text/css" href="Recetas.css"/> 
</head>
    
<body>
<xsl:for-each select="receta">
<xsl:if test="not(ingredientes/ingrediente/@nombre='Leche') and not(ingredientes/ingrediente/@nombre='Queso') and not(ingredientes/ingrediente/@nombre='Huevo') and not(ingredientes/ingrediente/@nombre='Carne') and not(ingredientes/ingrediente/@nombre='Merluza')">
